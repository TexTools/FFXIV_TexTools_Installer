...
struct Options
{
  ...
  BYTE noInherit;
  BYTE noSidToName;
  Options() : noInherit(0), noSidToName(0) {}
};
...
static DWORD GetOwner(const SchemeType* scheme, TCHAR* path, ChangeMode mode, TCHAR* owner, TCHAR* domain, Options* options)
{
  ...
  ret = MyGetNamedSecurityInfo(scheme, path, si, &psd, options);
  if (ret != ERROR_SUCCESS)
  ...
  pSid = pSidOwner ? pSidOwner : pSidGroup;
  if (options->noSidToName)
  {
    ConvertSidToStringSidNoAlloc(pSid, owner);
  }
  else
  {
    if (!LookupAccountSid(NULL, pSid, owner, &dwOwner, domain, &dwDomain, &eUse))
...
static const SchemeType* PopFileArgs(TCHAR* path, Options* options)
{
  for (;;)
  {
    if (popstring(path))
     ...
     if (!lstrcmpi(path, TEXT("/noinherit")))
      options->noInherit |= TRUE;
     else if (!lstrcmpi(path, TEXT("/sid")))
      options->noSidToName |= TRUE;
    else
      break;
  }
...
static BOOL PopRegKeyArgs(TCHAR* path, Options* options, TCHAR* param)
{
  ...
  for (;;)
  {
    ...
    if (!lstrcmpi(param, TEXT("/noinherit")))
      options->noInherit |= TRUE;
    else if (!lstrcmpi(param, TEXT("/sid")))
      options->noSidToName |= TRUE;
    else
      break;
  }
